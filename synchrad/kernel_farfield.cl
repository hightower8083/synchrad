  ...
  __global        uint *itSnaps)
{
  ...
  if (gti < nTotalChunk)
  {
    ...
    for (uint iCh=0; iCh < ${nChunk}; iCh++) {
      ...
      spectrLocalRe[iCh] = (${my_dtype}3) {0., 0., 0.};
      phaseGood[iCh] = false;
    }
      ...
      if (it<nSteps-1)
      {
        ...
        if (HasGoodCells)
        {
          ...
          for (uint iCh=0; iCh<${nChunk}; iCh++)
          {
            if (phaseGood[iCh])
            {
              sinPhase = ${f_native}sin(phase[iCh]);
              cosPhase = ${f_native}cos(phase[iCh]);

              spectrLocalRe[iCh] += amplitude * cosPhase;
              spectrLocalIm[iCh] += amplitude * sinPhase;
            }
          }
        }
      }
      ...
      if (it_glob+2 == itSnaps[iSnap])
      {
        for (uint iCh=0; iCh<${nChunk}; iCh++)
        {
          spectrum[gtiGlobal + iCh + nTotal*iSnap] +=  wpdt2 * (
            ...
            dot(spectrLocalIm[iCh], spectrLocalIm[iCh]) );
        }
        iSnap += 1;
      }
    }
